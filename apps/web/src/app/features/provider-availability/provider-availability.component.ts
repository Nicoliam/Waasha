...
/**
 * Slice 8 — Provider scheduling UI.
 * Weekly working hours + date exceptions, backed by /api/v1/providers/me/availability.
 * The booking engine stays authoritative: this screen never generates slots.
 * Offline mutations are blocked (never faked); only server-confirmed state is shown.
 */
@Component({
  selector: 'waasha-provider-availability',
  standalone: true,
    ...
    <div class="wa-avail">
      <header class="wa-avail__head">
        <div>
          ...
          <p class="wa-sub">
            Set your weekly working hours and exceptions. Future booking slots follow this schedule;
            existing bookings are never moved or cancelled by schedule changes.
          </p>
          <p class="wa-tz" *ngIf="timezone">Scheduling timezone: <strong class="wa-mono">{{ timezone }}</strong></p>
        </div>
        <a routerLink="/provider/bookings" class="wa-btn wa-btn-ghost wa-btn--sm">Booking inbox</a>
      </header>
      ...
      <div *ngIf="conflicts.length > 0 && !loading" class="wa-card wa-conflicts" role="alert">
        <p class="wa-conflicts__title">Schedule conflicts with existing bookings</p>
        <ul>
      ...
      <div *ngIf="!loading && !loadError">
        <section class="wa-card" aria-label="Weekly schedule">
          ...
          <div *ngFor="let day of days; let d = index" class="wa-day">
            <div class="wa-day__head">
              <strong>{{ day }}</strong>
              <button type="button" class="wa-btn wa-btn-ghost wa-btn--sm" [disabled]="offline || saving" (click)="startAdd(d)">Add hours</button>
            </div>
            <ul *ngIf="grouped[d].length > 0" class="wa-windows">
              <li *ngFor="let w of grouped[d]" class="wa-window">
                ...
                <button type="button" class="wa-btn wa-btn-ghost wa-btn--sm" [disabled]="offline || saving" (click)="toggleRule(w)">
                  {{ w.isActive ? 'Pause' : 'Resume' }}
                </button>
                <button type="button" class="wa-btn wa-btn-ghost wa-btn--sm" [disabled]="offline || saving" (click)="removeRule(w)">Remove</button>
              </li>
            </ul>
            ...
            <div *ngIf="addingDay === d" class="wa-add">
              ...
              <div class="wa-add__actions">
                <button type="button" class="wa-btn wa-btn-primary wa-btn--navy wa-btn--sm" [disabled]="offline || saving" (click)="saveNewWindow(d)">Save</button>
                <button type="button" class="wa-btn wa-btn-ghost wa-btn--sm" (click)="cancelAdd()">Cancel</button>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="wa-card" aria-label="Exceptions">
          ...
          <p class="wa-hint">Block full days (holidays, leave) or set special hours for a specific date.</p>
          <ul *ngIf="exceptions.length > 0" class="wa-windows">
            <li *ngFor="let e of exceptions" class="wa-window">
              <span>{{ exceptionText(e) }}</span>
              <span *ngIf="e.reason" class="wa-muted">— {{ e.reason }}</span>
              <button type="button" class="wa-btn wa-btn-ghost wa-btn--sm" [disabled]="offline || saving" (click)="removeException(e)">Remove</button>
            </li>
          </ul>
          ...
          <div class="wa-add">
            ...
            <div class="wa-add__actions">
              <button type="button" class="wa-btn wa-btn-primary wa-btn--navy wa-btn--sm" [disabled]="offline || saving" (click)="saveException()">Add exception</button>
            </div>
          </div>
        </section>
        ...
        <section class="wa-card" aria-label="Timezone">
          ...
          <p class="wa-hint">All hours are interpreted in this timezone. Booking slots are generated from it.</p>
          <div class="wa-add">
            <label>IANA timezone <input type="text" [(ngModel)]="tzEdit" aria-label="Timezone" placeholder="Africa/Johannesburg" /></label>
            <div class="wa-add__actions">
              <button type="button" class="wa-btn wa-btn-primary wa-btn--navy wa-btn--sm" [disabled]="offline || saving" (click)="saveTimezone()">Save timezone</button>
            </div>
          </div>
        </section>
...
export class ProviderAvailabilityComponent implements OnInit {
  ...
  saveNewWindow(day: number): void {
    ...
    this.mutate(
      this.api.createRule({ dayOfWeek: day, startTime: this.newStart, endTime: this.newEnd }),
      'Hours saved.',
    );
    this.addingDay = null;
  }
