      ...
      <form *ngIf="!loading && !loadError" (ngSubmit)="save()" #f="ngForm" novalidate>
        <section class="wa-card wa-prof__img" aria-label="Profile image">
          <div class="wa-prof__imgrow">
            ...
            <div>
              <label class="wa-label" for="profileImageUrl">Profile image reference</label>
              <input
                id="profileImageUrl"
              ...
              <div class="wa-upload" aria-label="Profile image upload">
                <label class="wa-label" for="profileImageFile">Upload a profile image</label>
                <input
                  id="profileImageFile"
                ...
                <p *ngIf="uploadError" class="wa-error__msg" role="alert">
                  {{ uploadError }}
                  <button type="button" class="wa-btn wa-btn-ghost wa-btn--sm" (click)="retryUpload()" [disabled]="uploading">Retry</button>
                </p>
        ...
        <section class="wa-card" aria-label="Editable profile fields">
          <label class="wa-label" for="displayName">Display name</label>
          <input id="displayName" name="displayName" type="text" class="wa-input" [(ngModel)]="form.displayName" maxlength="80" required />

          <label class="wa-label" for="bio">Bio</label>
          <textarea id="bio" name="bio" class="wa-input" rows="4" [(ngModel)]="form.bio" maxlength="2000"></textarea>

          <label class="wa-label" for="experienceSummary">Experience summary</label>
          <textarea id="experienceSummary" name="experienceSummary" class="wa-input" rows="3" [(ngModel)]="form.experienceSummary" maxlength="2000"></textarea>
        ...
        <div class="wa-actions">
          <button type="submit" class="wa-btn wa-btn-primary wa-btn--navy" [disabled]="saving || offline">
            {{ saving ? 'Saving…' : 'Save changes' }}
          </button>
          <button type="button" class="wa-btn wa-btn-ghost" (click)="reset()" [disabled]="saving">Cancel</button>
        </div>
